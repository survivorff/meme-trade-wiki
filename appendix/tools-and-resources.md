# 工具与资源

> Meme 交易世界的实用工具、数据源和学习资料。

---

## 发射台（Launchpad）

| 名称 | 链 | 说明 |
|------|-----|------|
| [Pump.fun](https://pump.fun) | Solana | 最大的 Meme 币发射台，累计收入 $10 亿+ |
| [LetsBonk.fun](https://letsbonk.fun) | Solana | BONK 生态发射台，2025 年中一度超越 Pump.fun |
| [Moonshot](https://moonshot.money) | Solana | DEXScreener 推出的移动端发射台，支持法币购买 |
| [Four.meme](https://four.meme) | BSC | BSC 链上的主要 Meme 发射台，对标 Pump.fun |
| [Zora](https://zora.co) | Base | 内容币发射台，社交媒体帖子 Token 化 |
| [Clanker](https://clanker.world) | Base、Arbitrum、BNB | AI Agent 驱动的多链发射台 |

## 交易平台

| 名称 | 类型 | 说明 |
|------|------|------|
| [Axiom](https://axiom.trade) | Web | 市场份额最高（~42%），集成交易+合约+挖矿 |
| [GMGN](https://gmgn.ai) | Web + TG Bot | Smart Money 追踪 + AI 分析，多链支持 |
| [Photon](https://photon-sol.tinyastro.io) | Web | 早期头部平台 |
| [BullX](https://bullx.io) | Web | 多链支持 |
| [Trojan](https://t.me/troaboronkov) | Telegram Bot | Telegram 原生交易机器人 |
| [BONKbot](https://t.me/bonkbot_bot) | Telegram Bot | BONK 生态交易机器人 |
| [Maestro](https://maestrobots.com) | Telegram Bot | 以太坊生态的主流交易机器人 |
| [Banana Gun](https://bananagun.io) | Telegram Bot | 以太坊狙击机器人 |

## 数据与分析

| 名称 | 说明 |
|------|------|
| [DEXScreener](https://dexscreener.com) | 链上交易数据、图表、新币发现 |
| [Birdeye](https://birdeye.so) | Solana 生态数据分析 |
| [Dune Analytics](https://dune.com) | 链上数据查询和仪表盘 |
| [Solscan](https://solscan.io) | Solana 区块浏览器 |
| [SolanaFM](https://solana.fm) | Solana 交易解析 |
| [Arkham](https://arkham.com) | 链上情报和钱包标签 |
| [Bubblemaps](https://bubblemaps.io) | Token 持仓分布可视化 |

## MEV 与基础设施

| 名称 | 说明 |
|------|------|
| [Jito](https://jito.wtf) | Solana MEV 基础设施，Bundle 提交 |
| [Helius](https://helius.dev) | Solana RPC + Webhook + DAS API |
| [QuickNode](https://quicknode.com) | 多链 RPC 服务商 |
| [Triton](https://triton.one) | Solana 高性能 RPC |
| [Chainstack](https://chainstack.com) | 多链节点服务 |

## 安全检测

| 名称 | 说明 |
|------|------|
| [RugCheck](https://rugcheck.xyz) | Solana Token 安全检测 |
| [GoPlus](https://gopluslabs.io) | 多链 Token 安全 API |
| [De.Fi](https://de.fi) | DeFi 安全扫描和审计数据库 |

## 学习资料

| 名称 | 说明 |
|------|------|
| [Solana 官方文档](https://solana.com/docs) | Solana 开发者文档 |
| [Solana Cookbook](https://solanacookbook.com) | Solana 开发实用指南 |
| [Rekt News](https://rekt.news) | DeFi 安全事件报道 |
| [Jito 文档](https://docs.jito.wtf) | Jito Bundle 和 MEV 文档 |

---

*持续更新中。欢迎提 Issue 推荐好用的工具。*
